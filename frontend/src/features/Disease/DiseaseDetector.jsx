            ...
            {!analysisResult ? (
                        ...
                        >
                          {isLoading ? (
                            <>
                              ...
                              {analysisStage === 2 && "Analyzing image..."}
                            </>
                          ) : (
                            <>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div className="lg:col-span-2">
                  <div className="bg-white p-6 rounded-lg shadow-sm border border-amber-100 h-full">
                    <h3 className="text-lg font-semibold mb-4 flex items-center">
                      <MdOutlineHealthAndSafety className="mr-2 text-green-500" />
                      Analysis Results
                    </h3>
                    <div className="bg-green-50 p-5 rounded-lg border border-green-100 h-[calc(100%-3rem)] overflow-y-auto">
                      <div className="prose prose-green prose-sm max-w-none">
                        {analysisResult.split('\n').map((line, idx) => {
                          if (line.trim().startsWith('**') && line.trim().endsWith('**')) {
                            return <h4 key={idx} className="font-bold text-green-800 mt-4 mb-2 text-lg border-b border-green-200 pb-1">{line.replace(/\*\*/g, '')}</h4>;
                          }
                          if (line.trim().startsWith('*') || line.trim().startsWith('-')) {
                            return <li key={idx} className="ml-4 text-gray-700 mb-1">{line.substring(1).trim()}</li>;
                          }
                          if (line.trim() === '') {
                            return <br key={idx} />;
                          }
                          return <p key={idx} className="text-gray-700 mb-2">{line.replace(/\*\*/g, '')}</p>;
                        })}
                      </div>
                    </div>
                  </div>
